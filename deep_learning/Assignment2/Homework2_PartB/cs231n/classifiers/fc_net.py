...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        ############################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        cur_layer_dim = input_dim
        for i, hd in enumerate(hidden_dims):
            self.params['W%d'%(i+1)] = weight_scale * np.random.randn(cur_layer_dim, hd)
            cur_layer_dim = hd
            ...
            if self.normalization == "batchnorm":
                ...
                self.params['gamma%d'%(i+1)] = np.ones(hd)
                self.params['beta%d'%(i+1)] = np.zeros(hd)
        self.params['W%d'%(self.num_layers)] = weight_scale * np.random.randn(cur_layer_dim, num_classes)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            if self.normalization == "batchnorm":
                ...
                layer_input, cache[i] = affine_ln_relu_forward(layer_input,
                                            self.params['W%d'%i], self.params['b%d'%i], 
                                            self.params['gamma%d'%i], self.params['beta%d'%i], self.bn_params[i-1])
            else:
                layer_input, cache[i] = affine_relu_forward(layer_input, self.params['W%d'%i], self.params['b%d'%i])
        ...
        for idx in range(1, self.num_layers):
            ...
            if self.normalization == "batchnorm":
                dx, dw, db, grads['gamma%d'%i], grads['beta%d'%i] = affine_bn_relu_backward(dout, cache[i])
            elif self.normalization == "layernorm":
                dx, dw, db, grads['gamma%d'%i], grads['beta%d'%i] = affine_ln_relu_backward(dout, cache[i])
            else:
