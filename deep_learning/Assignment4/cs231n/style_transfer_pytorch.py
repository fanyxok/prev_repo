...
def content_loss(content_weight, content_current, content_original):
    ...
    sum_diff2 = torch.sum(diff * diff) 

    content_loss_ = content_weight * sum_diff2
...
def gram_matrix(features, normalize=True):
    ...
    gram = features.matmul(features_T)
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i in range(len(style_layers)):
      style_loss_ += content_loss(style_weights[i], gram_matrix(feats[style_layers[i]].clone()), style_targets[i])
    return style_loss_
    pass
...
def tv_loss(img, tv_weight):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    left_side = torch.sum((img[:,:,:,1:] - img[:,:,:,:-1])**2)
    right_side = torch.sum((img[:,:,1:] - img[:,:,:-1])**2)
    tv_loss_ = tv_weight * (left_side + right_side)
    return tv_loss_
    pass
