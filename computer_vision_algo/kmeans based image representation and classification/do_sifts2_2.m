function [sift_f_mat, sift_d_mat,feature_num] = do_sifts2(class_number,train_number,test_number,imgdir)
...
for i=1:class_number
    i
    for j=train_number+1:train_number+test_number
        if i<10 
            if j < 10
        ...
        [sift_f, sift_d] = vl_dsift(img_gray,'size',4, 'step',4);
        [~, n] = size(sift_d);
        feature_num = [feature_num n];
        sift_d_mat = [sift_d_mat sift_d];
        sift_f_mat = [sift_f_mat sift_f(1:2,:)];
    end
end
